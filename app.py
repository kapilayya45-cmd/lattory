...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'smart-win-ultra-final-v5'

# --- మీ అమేజాన్ ఇమేజ్ లింక్స్ ఇక్కడ సెట్ చేశాను ---
CATEGORIES = {
    'low_cost': {
        'name': 'Budget Kings', 
        'model': 'Redmi 13C', 
...
# --- UI Template (Form Overlap Fix + Image Visibility) ---
USER_HTML = """
<!DOCTYPE html>
<html lang="te">
<head>
    ...
    <style>
        ...
        .hero h1 { font-family: 'Syncopate', sans-serif; font-size: 2.2rem; color: #00f2ff; text-shadow: 0 0 15px #00f2ff; letter-spacing: 5px; text-transform: uppercase; }
        
        /* Neon Timer */
        .timer-box { background: rgba(255,255,255,0.03); border: 1px solid #00f2ff; padding: 10px 25px; border-radius: 50px; display: inline-block; margin-top: 15px; box-shadow: 0 0 15px rgba(0,242,255,0.2); }
        #countdown { font-size: 1.6rem; font-weight: 800; color: #fff; letter-spacing: 2px; }

        /* Split Card Design: Right(Image), Left(Details) */
        .card-split { 
            background: #111; 
        ...
        /* Booking Alert Box */
        .booking-alert { background: #111; border: 1px solid #28a745; color: #fff; border-radius: 25px; padding: 30px; margin-bottom: 40px; box-shadow: 0 0 20px rgba(40,167,69,0.2); }
...
<body>
    ...
    <div class="container mt-5">
        {% with messages = get_flashed_messages() %}{% if messages %}{% for m in messages %}
            <div class="booking-alert text-center shadow-lg">
                <h3 class="text-success fw-bold">🎉 టికెట్ రిజర్వ్ అయ్యింది!</h3>
                <p class="mb-3">{{m}}</p>
                <hr style="border-color: #333;">
                <p class="mb-3">టికెట్ కన్ఫర్మ్ చేయడానికి ఈ QR కోడ్ స్కాన్ చేసి పేమెంట్ చేయండి:</p>
                <div class="bg-white p-3 d-inline-block rounded-3 mb-3">
                    <img src="https://qrserver.com" alt="QR">
                </div>
                <p class="text-secondary small">పేమెంట్ అయ్యాక స్క్రీన్ షాట్ వాట్సాప్ (9121195323) కి పంపండి.</p>
            </div>
        ...
        <div class="card-split shadow">
            ...
            <div class="detail-side">
                ...
                <form action="/buy/{{ key }}" method="POST">
                    <input name="name" placeholder="పూర్తి పేరు (FULL NAME)" class="form-control" required>
                    <input name="phone" placeholder="వాట్సాప్ నంబర్ (WHATSAPP NO)" class="form-control" required>
                    <textarea name="address" placeholder="డెలివరీ అడ్రస్ (DELIVERY ADDRESS)" class="form-control" rows="2" required></textarea>
