...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'smart-win-absolute-fix-2024'

# --- Images Fix: Direct Reliable Links ---
CATEGORIES = {
    'low_cost': {
        'name': 'Budget Kings', 
        'model': 'Moto G45 5G', 
...
<html>
<head>
    <title>SMART-WIN | Final Fix</title>
    ...
    <style>
        ...
        .card-split { 
            ...
            overflow: hidden; margin-bottom: 40px; display: flex; 
            flex-direction: row-reverse; flex-wrap: wrap; transition: 0.3s;
        }
        
        /* Image Side - Fixed logic to show images */
        .img-side { 
            flex: 1; background: #ffffff; display: flex; align-items: center; 
            justify-content: center; min-width: 300px; height: 400px; padding: 20px;
        }
        .mobile-img { 
        ...
        @media (max-width: 768px) { .card-split { flex-direction: column; } }
    ...
    <div class="container mt-5">
        {% with messages = get_flashed_messages() %}{% if messages %}{% for m in messages %}
            <div class="text-center p-4 mb-4" style="background:#111; border: 1px solid #28a745; border-radius:25px;">
                ...
                <p>Scan & Send Screenshot to 9121195323</p>
            </div>
        ...
        <div class="card-split shadow-lg">
            <div class="img-side">
                <img src="{{ info.image_url }}" referrerpolicy="no-referrer" class="mobile-img" alt="{{ info.model }}">
            </div>
