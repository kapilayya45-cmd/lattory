...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'smart-win-ultra-fix-999'

# --- IMAGE PROXY (To bypass Amazon/PNG blocking) ---
def get_safe_url(url):
    return f"https://weserv.nl{url}&w=500"

# --- Hardcoded Data (Since Vercel doesn't save Database changes) ---
CATEGORIES = {
    'low_cost': {
        'name': 'Budget Kings', 
        'model': 'Feature Phone HD', 
        'specs': 'Reliable Battery | Dual Sim | Best for Daily Use',
        'price': 50, 
        'color': '#00f2ff',
        'image_url': get_safe_url('://pngitem.com')
    },
    'mid_range': {
        'name': 'Mid-Range Beasts', 
        'model': 'Samsung Galaxy Series', 
        'specs': 'Super AMOLED Display | 5G Network | High Speed RAM',
        'price': 150, 
        'color': '#b721ff',
        'image_url': get_safe_url('://pngfind.com')
    },
    'flagship': {
        ...
        'price': 500, 
        'color': '#ff4b2b',
        'image_url': get_safe_url('://media-amazon.com')
    }
}
...
<head>
    ...
    <style>
        ...
        .img-side { flex: 1; background: #fff; display: flex; align-items: center; justify-content: center; padding: 25px; min-width: 300px; min-height: 380px; }
        ...
        .form-control { background: #1a1a1a !important; border: 1px solid #333 !important; color: #fff !important; border-radius: 12px; margin-bottom: 12px; padding: 12px; }
        ...
        @media (max-width: 992px) { .card-split { flex-direction: column; } .img-side { min-height: 280px; } }
    ...
    <div class="container mt-5">
        {% with messages = get_flashed_messages() %}{% if messages %}{% for m in messages %}
            <div class="text-center p-4 mb-4" style="background:#111; border: 1px solid #28a745; border-radius:25px;">
                <h4 class="text-success">🎉 TICKET RESERVED!</h4>
                <div class="bg-white p-3 d-inline-block rounded mb-2">
                    <img src="https://qrserver.com" alt="QR">
                </div>
                <p class="small">Pay & WhatsApp Screenshot to 9121195323</p>
            </div>
        ...
        <div class="card-split">
            <div class="img-side">
                <img src="{{ info.image_url }}" referrerpolicy="no-referrer" class="mobile-img" alt="Mobile">
            </div>
